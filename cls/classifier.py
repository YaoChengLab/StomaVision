...
def train(opt, device):
    ...
    with torch_distributed_zero_first(LOCAL_RANK):
        if opt.model == "list":
            ...
            # print(model.save)
            model = ClassificationModel(
                model=model, nc=nc, cutoff=opt.cutoff or 52
            )  # classification model
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            if RANK in {-1, 0}:
                ...
                if i == len(pbar) - 1:  # last batch
                    top1, top5, vloss = test(
                        ema.ema, testloader, names, criterion, pbar=pbar
                    )  # test accuracy, loss
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--model", type=str, default="yolov5s", help="initial weights path"
    )
    parser.add_argument(
        "--data",
        type=str,
        default="mnist",
        help="cifar10, cifar100, mnist or fashion-mnist",
    )
