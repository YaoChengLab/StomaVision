...
def run(
    ...
    connect_mask=False,  # use dilation-erosion technique to link disconnected masks
    dilation=25,  # dilation pixels
    erosion=10,  # erosion pixels
):
    source = str(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):  # per detection
                ...
                if connect_mask:
                    masks = dilation_erosion_tensor_masks(
                        masks, 24, 2
                    )  # dilation and eroson to connect mask
                im_masks = plot_masks(
                    im[i], masks, mcolors
                ...
                # ABRC: save masks to rotated bboxes ---------------------------------------------------------------
                scale_h = im0.shape[0] / im.shape[2:][0]
                scale_w = im0.shape[1] / im.shape[2:][1]
                scale_ratio = scale_w / scale_h
                with open(f"{txt_path}.txt", "a") as f:
                    f.write("rotated_bbox\n")

                LOGGER.info("RBBOX")
                LOGGER.info(
                    f"scale h = {scale_h}, w = {scale_w}, ratio = {scale_ratio}"
                )
                LOGGER.info(f"im0 h = {im0.shape[0]} w = {im0.shape[1]}")
                LOGGER.info(f"img h = {im.shape[2:][0]} w = {im.shape[2:][1]}")

                fitted_rbboxes = []
                categories = []

                for mask, cls in zip(masks, det[:, 5]):
                    # Convert a binary mask to polygon
                    polygons = binary_mask_to_polygon(mask)

                    # Scale mask to orginal image
                    # YOLOv7 change adjust image height and width to fultil its input requirements
                    # Code below adjust the
                    for j, p in enumerate(polygons):
                        scales = [
                            scale_h if i % 2 != 0 else scale_w for i in range(len(p))
                        ]
                ...
                # Draw rbbox on image
                img_file = path
                img_draw = draw_rotated_bboxes(
                    img_file, fitted_rbboxes, categories, thickness=2, color=None
                )
...
def parse_opt():
    ...
    parser.add_argument(
        "--connect-mask",
        action="store_true",
        help="use dilation and erosion to link disconnected masks",
    )
    parser.add_argument("--dilation", type=int, default=10, help="dilation pixels")
    parser.add_argument("--erosion", type=int, default=10, help="erison pixels")
